...
if username != 'stobsy' and username != 'admin':
    ...
    def aircraft_manager():
        ...
        st.info("To edit aircraft parameters please contact EFB Manager - this functionality will be implimented at the user level soon")
        con = db_util.con_db(config_file['aircraft']['database'])
        cur = con.cursor()
    ...
    def aircraft_importer():
        ...
        db_filename = config_file['aircraft']['database']
    ...
    def edit_data():
        ...
        st.info("Database in use: " + config_file['database'])
        ...
        if st.button('Load Profile'):
            ...
            st.session_state.edit_perf_profile = pd.read_sql(query, con)
        ...
        if st.session_state.edit_perf_profile.empty:
            st.session_state.edit_perf_profile = add_row(st.session_state.edit_perf_profile , rowid)
    ...
    def set_data():
        st.header("Add Raw Perf Data")
        db_folder = os.path.abspath(os.path.join(os.path.dirname( __file__ ), '..', 'db'))
        data_folder = os.path.abspath(os.path.join(os.path.dirname( __file__ ), '..', 'data'))
        data_folder
        sqlite_dbs = [file for file in os.listdir(db_folder) if file.endswith('.db')]
        db_filename = st.selectbox('DB Filename', sqlite_dbs)
        data_files = [file for file in os.listdir(data_folder) if file.endswith('.txt')]
        data_filename = st.selectbox('DB Filename', data_files)
        data = file_util.load_file(data_filename, data_folder)
        con = db_util.con_db(db_filename)
        

        con.row_factory = lambda cursor, row: row[0]
        cur = con.cursor()
        cur.execute("SELECT name FROM perf_profiles")  # execute a simple SQL select query
        aircraft = cur.fetchall()  # get all the results from the above query
        recorded_for = st.selectbox('Aircraft to apply data to', aircraft)
        

        if st.button('Add perf_data_set'):
            cur.execute("SELECT rowid, * FROM perf_profiles WHERE name = ('" + recorded_for +"')")
            rowid = cur.fetchone()
            rowid
            for d in data:
                #(ID, Weight, Altitude, CruiseTas, CruiseBurn, ClimbTas, ClimbBurn, ClimbRate, TAT, SAT)
                cur.execute("INSERT INTO perf_data VALUES(?, ?, ?, ?, ?, ?, ?, ?, ?, ?)", (rowid, d[0], d[1], d[2], d[3], d[4], d[5], d[6], 0, 0) )
                con.commit()
    ...
    def run_query():
        ...
        conn = db_util.create_connection(db_filename)
    ...
    def settings():
        st.header("Aircraft Manager Settings")
        ...
        if (config_file["database"] in databases):
            with st.form("settings_form"):
                ...
                submitted = st.form_submit_button("Submit",on_click=submit_db, args=(database,))
            if submitted:
                config_file["database"] = database
                st.success("Database changed")
            ...
            with st.form("database_select"):
                st.error("database not found - please select database to use")
                database = st.selectbox('Data Base in Use', databases)
                submitted = st.form_submit_button("Submit" , on_click=submit_db, args=(database,))
            if submitted:
                config_file["database"] = database
                st.success("Database set")
    ...
    page_names_to_funcs = {
        "Aircraft Importer" : aircraft_importer,
        "Aircraft Manager": aircraft_manager,
    }
